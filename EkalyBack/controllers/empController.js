...
exports.getEmployeByDate = async(req, res, next) => {
    ...
    await Employe.find({ dateCreated: { "$gte": new Date(currentDay), "$lt": new Date(nextDay) } })
...
exports.doEmpCheck = async(req, res, next) => {
    ...
    if (typeChecking == 0) {
        ...
        let timeEnter;
        await CheckEmp.findOne({ employeId: employeId, dateCheck: { "$gte": new Date(currentDay), "$lt": new Date(nextDay) } })
            .then((checkEmploye) => {
        ...
        let calcDiffTime = moment.duration(timeLeaveFormatted.diff(timeEnterFormatted));
        let diffTime = calcDiffTime.hours() * 60 + calcDiffTime.minutes() + calcDiffTime.seconds() / 60; // diffTime in minutes
